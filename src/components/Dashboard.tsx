...
import JorgeTour from './JorgeTour';
...
export default function Dashboard() {
    ...
    const [jorgeReady, setJorgeReady] = useState(false);
    ...
    useEffect(() => {
        ...
        // Jorge: client-only state — never runs on the server
        setJorgeReady(true);
        const tourSeen = localStorage.getItem('jorge_tour_seen');
        if (!tourSeen) {
            setShowTour(true);
        }
    ...
    return (
        <div className="flex flex-col md:flex-row h-screen overflow-hidden bg-background-dark text-slate-100 font-display relative">
            ...
            {/* JORGE — CLIENT-ONLY RENDER (Fix Error #418 + idle viewport) */}
            {jorgeReady && (
                <div
                    style={{
                        position: 'fixed',
                        bottom: '120px',
                        left: '70px',   // Fix 2.10: final margin to prevent lamp cutoff
                        zIndex: 150,
                    }}
                >
                    <div className="relative group" style={{ minWidth: '130px' }}>
                        <StudyLamp isJorgeMode={showTour} />
                        {!showTour && (
                            <button
                                onClick={() => {
                                    localStorage.removeItem('jorge_tour_seen');
                                    setShowTour(true);
                                }}
                                className="absolute top-4 -right-2 p-1.5 bg-slate-800 border border-white/10 rounded-full text-slate-400 hover:text-[#f4a261] hover:border-[#f4a261] transition-all scale-75 shadow-lg group-hover:scale-110 z-[160]"
                                title="Jorge opnieuw zien"
                            >
            ...
            <aside className={`fixed md:relative inset-y-0 left-0 w-[240px] flex-shrink-0 border-r border-slate-800/50 bg-background-dark flex flex-col p-6 z-50 transition-transform duration-300 md:translate-x-0 ${isSidebarOpen ? 'translate-x-0' : '-translate-x-full'}`}>
                ...
                <div className="flex flex-col gap-1 flex-grow overflow-y-auto custom-scrollbar">
                  ...
                  ${isActive
                                        ...
                                        : unit.isLocked
                                            ? 'text-slate-600 cursor-not-allowed opacity-50 hover:bg-slate-800/50'
                ...
                <div className="mt-auto pt-6 border-t border-slate-800/50 space-y-3">
                    <div id="xp-bar" className="flex items-center gap-3 px-3 py-2 rounded-xl bg-accent-gold/10 border border-accent-gold/20">
            ...
            <main className="flex-grow overflow-y-auto custom-scrollbar bg-background-dark p-4 md:p-8 pb-32 md:pb-8">
                <div className="max-w-4xl mx-auto">
                    {loading ? (
                        ...
                        <>
                            <header className="mb-10" id="ppp-container">
                                ...
                                <div className="bg-slate-800/30 rounded-full h-2 w-full relative overflow-hidden shadow-inner">
                                    <div
                                        className="absolute top-0 left-0 h-full bg-primary rounded-full transition-all duration-1000 shadow-[0_0_12px_rgba(255,107,107,0.4)]"
                            ...
                            <div className="flex flex-col gap-4" id="lessons-container">
                                {activeUnit?.lessons.map((lesson) => {
                                    ...
                                    return (
                                        <div
                                            key={lesson.id}
                                            className={`relative overflow-hidden transition-all duration-300 rounded-xl p-5 md:p-6 flex flex-col sm:flex-row items-center justify-between gap-4 md:gap-0
                        ${isCompleted
                                                    ? 'bg-[#121812] border-l-4 border-green-500 shadow-lg hover:shadow-xl'
                                                    : isUnlocked
                                                        ? 'bg-card-dark border-l-4 border-[#FF6B6B] shadow-2xl hover:bg-[#1f1f3a] hover:shadow-xl'
                                                        : 'bg-card-dark/40 border-l-4 border-slate-700/50 opacity-80 hover:shadow-lg'}`}
                                        >
            ...
            {/* 3. RIGHT PANEL */}
            <aside id="hulpkaart-container" className="relative w-[320px] flex-shrink-0 border-l border-slate-800/50 bg-background-dark p-8 flex flex-col hidden xl:flex overflow-y-auto custom-scrollbar">
      ...
      justify-center">
                ...
                <div className="grid grid-cols-2 gap-3 mb-8">
                    <div className="bg-card-dark p-4 rounded-xl border border-slate-800/50 hover:border-primary/40 hover:scale-[1.02] transition-all duration-200">
